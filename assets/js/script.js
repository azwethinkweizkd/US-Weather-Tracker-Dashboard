...
function getFromLocalStorage() {
	...
	for (let i = 0; i < prevSearch.length; i++) {
		let prevCityBtn = $("<button>")
			.addClass(
				"p-2 btn searchBtn btn-dark my-2 btn-block d-flex align-items-center justify-content-between"
			)
			.text(prevSearch[i]);

		// Create a delete button for each city
		let deleteButton = $("<button>")
			.addClass("btn btn-sm btn-danger deleteBtn")
			.text("X");

		prevCityBtn.append(deleteButton);
...
async function getAPI() {
	if (searchedCity.trim() !== "") {
		let requestURLCurr = `https://api.openweathermap.org/data/2.5/weather?q=${searchedCity}&appid=${apiKey}&units=imperial`;

		await fetch(requestURLCurr)
			.then((response) => response.json())
			.then((data) => searchedResponse(data));
	}
}
...
function searchedResponse(data) {
	...
	$("#day4").empty();
	$("#day5").empty();
	$("#currentDay").addClass("border border-dark rounded");
	...
	cityName.append(iconDisplay);
	currentDay.append(cityName);
	let temp = $("<p>")
		.addClass("p-2")
		.text("Temp: " + parseInt(data.main.temp_max) + " °F");
	currentDay.append(temp);
	let wind = $("<p>")
		.addClass("p-2")
		.text("Wind: " + parseInt(data.wind.speed) + " mph");
	...
	let requestURLFuture = `https://api.openweathermap.org/data/2.5/onecall?lat=${lat}&lon=${lon}&exclude=hourly,minutely&units=imperial&appid=${apiKey}`;

	fetch(requestURLFuture)
		.then(function (responseFuture) {
			return responseFuture.json();
		})
		.then(function (dataFuture) {
			...
			for (let i = 0; i < 6; i++) {
				...
				$(`#day${[i]}`).addClass("border border-dark rounded");
			}
		});
	getFromLocalStorage();
}
...
$(document).on("click", ".deleteBtn", function () {
	let cityNameToDelete = $(this).parent().text().replace("X", "").trim();
	prevSearch = prevSearch.filter((city) => city !== cityNameToDelete);
...
$("#searchedCities").on("click", ".searchBtn", function (event) {
	if (!$(event.target).hasClass("deleteBtn")) {
		let cityName = $(this).text().replace("X", "");
		searchedCity = cityName.trim();
		getAPI();
	}
});
